...
md"If the above prices we just filled in were plotted agains the profit we will be making against the corresponding price, the we will see the plot a below:"
...
md"The plot is starting to take some familiar shape, isn't it 🤔? Now if we add sufficient points to plot we can get some better shape, let us see what happens then."

# ╔═╡ 94849422-2b65-11eb-349a-299e4462a479
md"If you plot the price $$p_1$$ (per cup price of lemonade) chosen on the slider on the x-axis of the graph, versus your profit on the y-axis, you would see something like the plot below:"
...
md"This can be done by solving the following quadratic equation for $p_1$: $Profit = -2p_1^2 +20p_1$. Also for simplicity, let us call $Profit$ as $P$."
...
md"The coefficient of $p_1$, which we know to be -10, should be equal to $2a$. This means $a=$?"
...
md"We have both the solutions now, we can cpick whichever price from the above 2. Try choosing one of the options below to see if you made right choice of not."
...
if "2.763" ∈ opt
		md"Great work! 😀 This is because, no matter what price, from the above 2 choices, you pick, you will make the same profit. Remember the inverse parabola graph. If not take a look below!"
	else
		"Good choice, but not the right one! Try again!"
	end
...
# ╔═╡ 177d16f7-b96d-4593-b712-200abdd5e41e
...
md"Now we solve our quadratic equation using following formula: and get:"
...
md"Now if we add  sufficient points plot these on a graph, where our y-axis is the price of our company i.e., $p_1$ and the x-axis is $p_2$."
